...
### 场景A1：记录日常支出
1. 读取当前年度文件（如 `2026.md`）
2. 找到当前月份区块 `### MM月`
3. 追加到 **`#### 当月支出明细`**（禁止写入收入区块）
4. 运行 `python scripts/recalc.py --update` 更新面板
5. 运行 `python auto-sync.py` 同步 GitHub 与仪表盘

### 场景A2：记录工资/收入
1. 读取当前年度文件
2. 找到当前月份区块
3. 追加到 **`#### 当月收入`**（禁止与支出混记）
4. 格式：`- YYYY-MM-01 | 收入 | [资金流入] | #工资 | ￥3,100.00 | N月工资`
5. 工资默认 **￥3,100**，日期 **该月 1 日**（用户指定则以用户为准）
6. 运行 `python scripts/recalc.py --update` 后 `auto-sync.py`
...
| 收入分区 | 收入写 `#### 当月收入`，支出写 `#### 当月支出明细` | 统计与仪表盘失真 |
| 工资日期 | `#工资` 默认记在该月 **1 日** | 月度收入对齐困难 |
...
用户输入：6月工资
AI输出（写入 #### 当月收入）：
- 2026-06-01 | 收入 | [资金流入] | #工资 | ￥3,100.00 | 6月工资
```

...
## 2026-06-05 | 收入分区 v3.2 + 仪表盘收支升级

**背景**：收入与支出混记导致漏记工资；仪表盘仅展示支出。

**执行内容**：
1. 月度四段结构：`当月数据面板` + `当月收入` + `当月支出明细`
2. 2026 年 1～6 月工资统一 ￥3,100/月，入账日每月 1 日
3. `recalc.py` / `visualize.py` 增加支出占收入指标
4. GitHub Pages 仪表盘：KPI 含收入/结余/占比，每月收支双柱图
5. 新增 `AGENTS.md`；更新 `SYSTEM.md` / README / new_year 模板

**Agent 入口**：先读 `AGENTS.md`，再读 `SYSTEM.md`。
